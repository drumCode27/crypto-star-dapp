...
contract StarNotary is ERC721 {
    ...
    function exchangeStars(uint256 _tokenId1, uint256 _tokenId2) public {
        ...
        require(bytes(star1.name).length > 0, "No star exists for _tokenId1!");

        Star memory star2 = tokenIdToStarInfo[_tokenId2];
        require(bytes(star2.name).length > 0, "No star exists for _tokenId2!");

        address ownerOfToken1 = ownerOf(_tokenId1);
        address ownerOfToken2 = ownerOf(_tokenId2);
        //1. Passing to star tokenId you will need to check if the owner of _tokenId1 or _tokenId2 is the sender
        require(ownerOfToken1 == msg.sender || ownerOfToken2 == msg.sender, "Sender must own star for _tokenId1 or _tokenId2!");
    ...
    function transferStar(address _to1, uint256 _tokenId) public {
        ...
        require(msg.sender == ownerOf(_tokenId), "Sender must own star for tokenId!");
